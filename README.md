...
这是本项目的**自有规则语言**（`text/`），也是 1.1.8 那套「替换 + 加喵 + 颜文字 + 猫爪」的归宿。
1.1.8 把这三件事写死在服务里：后缀是一个固定字段、颜文字是一串常量、**QQ 猫爪是一段只有 QQ 字体
画得出来的不可见字节**。这里它们全都是**变量**，也就是可自定义的配置项。

一行一条规则，从上往下依次套用：

```
# 1. 简单替换 —— 和 1.1.8 的写法一模一样，原有规则原样粘贴即可
你好 = 您好
おはよう → 早安
...
# 4. 正则、概率、每 N 次
当 匹配/\d{4}-\d{2}-\d{2}/ 则 正则替换/(\d{4})-(\d{2})-(\d{2})/ 为 "$1年$2月$3日"
当 随机 30% 则 加颜文字
当 每 3 次 则 末尾加"{后缀}"

# 5. 只在某个应用里生效（写包名；编辑器里有微信/QQ 的模板按钮）
末尾加"{猫爪}" @com.tencent.mobileqq
...
按应用限定写的是**包名**（`@com.tencent.mm`）。表单编辑器里可以直接勾微信/QQ/微信输入法，也可以
自己填任意包名 —— 没做「列出所有已安装应用」的选择器：那需要 `QUERY_ALL_PACKAGES`，为一个替换规则
去要「读取所有应用」的权限不划算。

**条件**只有一小撮：`含 / 不含`、`以…开头 / 结尾`（可加「不」）、`是 / 不是`、`匹配/正则/`、
`长度 > N / < N`、`随机 N%`、`每 N 次`。**动作**同样：`A = B`（替换）、`删除`、`开头加`、
`末尾加`、`每句末尾加`、`首尾包裹`、`去掉末尾标点`、`加颜文字`、`正则替换`。

几条设计上的选择：

- **幂等**：已经有后缀就不再加，已经首尾包裹就不再包，所以连点两下不会变成「喵喵喵」；
- **`每句末尾加`只认含中文/日文/韩文的句子**（1.1.8 的旧经验：否则「喵」会钻到颜文字里面去）；
- **引号外的关键字才算关键字**：`替换 "因为" 为 "所以"` 里的「为」不会被当成分隔符 —— 中文本身
  就是这门语言最大的坑，解析器只在引号外找关键字；
- **一条坏行只损失那一行**，其余照常生效，编辑器按行号指出问题；
- 配置**以它自己的文本形式存储**（`text/TextPrefs.kt`）：渲染和解析互为逆运算（单元测试守着这条），
  所以编辑器里看到的就是存下来的，也就能直接粘给别人。

**什么时候生效**（不做实时改写，不碰输入法）：

1. 动作链里新增的 **套用规则** 一步 —— 悬浮窗按钮和发送按钮助手都能用，于是就有
   `AI 修改 → 套用规则 → 按发送` 这样的链；
2. **AI 修改之后自动套用**（默认开）：模型写完，规则收尾，一处实现（`util/AiRewrite.kt`），
   两个入口同时受益 —— 这就是「AI 修改后再替换」；
3. 纯规则、不调 AI 的玩法：助手动作选「套用规则」+ 自动发送，就是 1.1.8 那种「一键猫化并发送」。

**从 1.1.8 迁移**（`TextPrefs.importLegacy`）：1.1.8 与本是同包，升级安装后它的 `miao_config`
还在，可以直接读出来翻译成规则 —— 替换规则原样搬、加喵变成 `末尾加"{后缀}"`、空格不加变成
`（空格不加）`、标点优化变成 `去掉末尾标点`、自定义颜文字变成 `{颜文字}` 的值、QQ 猫爪变成
`首尾包裹"{猫爪}" @com.tencent.mobileqq`（值可改）、启用应用变成每条规则末尾的 `@包名`。

**出厂不带任何规则**：1.1.8 是默认开着加喵的，但这个应用的 AI 按钮从来没往里塞过东西，一个默默
开始改文本的功能会被卸载。1.1.8 的行为做成了预设包（猫化 / 猫爪），编辑器里一点就能用。

**页面按 1.1.8 的形状重做**（在此之前，页面上堆过开关、列表、两个入口、规则包、导入、试跑六摊
东西，被骂"界面混乱、多一个按钮是神经病"）：
...
文字处理
  末尾加后缀                       [开关]     ← 1.1.8 的「喵」
  句末颜文字                       [开关]
  QQ 猫爪                          [开关]     ← 只在 QQ 生效，和 1.1.8 一样
替换规则
  你好 = 您好                      [删除]     ← 点按改（原文 / 替换为）
  新增替换规则                                ← 也可以「粘贴一行」A = B
自定义颜文字                       [多行框]    ← 一行一个，留空用内置的 54 个
试跑                              [一句] [结果]
```

**开关背后的值不可改**（后缀就是「喵」、猫爪就是 1.1.8 那个印记），这是刻意的：能编辑的东西一多，
页面就没人看得懂。开关与规则的关系在 `text/TextSwitches.kt`（纯 Kotlin，有测试）：**关掉是禁用不是
删除**，再打开原样回来；猫爪按 1.1.8 限定在 QQ。

**引擎里的高级能力还在，只是界面上没有**：条件句、正则、变量编辑、规则包、按应用限定、纯文本 DSL
编辑器。这是"暂停"而不是"删除" —— 数据模型、解析器、25 个单元测试都留着，等界面想清楚了再放出来。

颜文字数据与 1.1.8 的 `BUILTIN_EMOTICONS` 逐条比对过：**54 条，内容与顺序完全一致**；
自定义颜文字按 1.1.8 的语义 —— 填了就整表替换内置，清空就回到内置。

表单与模型的互转是纯 Kotlin（`text/TextRuleForm.kt`），有测试守着「表单能显示每一条规则并原样还回来」
—— 一个把规则改坏的编辑器比没有编辑器更糟。

规则与规则包存在独立的 `text_rules` 偏好文件里，因此在配置备份范围内。内核是纯 Kotlin，
单元测试跑在 CI 上（`app/src/test/java/love/miao/yun/text/TextRulesTest.kt`）—— 语言这种东西
不能只靠「编译通过」来验证。

## 发送按钮助手

在**微信 / QQ** 的发送按钮上方放一个小按钮（`service/SendAssistOverlay.kt`）：点它先执行你选的
动作（默认 AI 修改），必要时再替你按下发送 —— 免去「文字被改写后还要自己找发送键」这一步。

- 它是 **`TYPE_ACCESSIBILITY_OVERLAY`**：挂在无障碍服务里，所以**不需要悬浮窗权限、也没有第二个
  前台服务与通知**，服务一停它就跟着消失；
- 位置每次都由无障碍树里发送按钮的屏幕坐标算出来，所以键盘弹出、输入栏变高、横竖屏切换都会跟上；
- **发送按钮的识别**先按 view id（微信 `com.tencent.mm:id/bql`、QQ `com.tencent.mobileqq:id/send_btn`），
  找不到就退化为「可见、可用、可点击且文字含「发送」的按钮里位置最靠下的那个」—— 微信的 id 每个
  版本都会重新混淆，只认 id 早晚会失效。QQ 里同时存在两个 `send_btn`（一个是相册面板里 `invisible`
  的），所以「可见」这个条件不是可选的；
- **默认不自动发送**。改写和发送是两件事，后者不可撤销，所以自动发送要单独打开；而且
  **改写失败时绝不会发送**（只有真的写回成功才会按）；
- **谁发的的事件不决定任何事**：每次事件只是让助手重新看一眼真实窗口状态。踩过的两个坑都在这：
  注入按钮这个动作本身会产生一条 `love.miao.yun` 的事件，输入法也在不停发事件，早期「不是目标
  应用就隐藏」的写法会让按钮在微信里闪一下就没；
- **窗口不只看前台那个**：键盘弹起时获得焦点的窗口可能不是聊天应用，`rootInActiveWindow` 也不
  一定是它，所以前台窗口不是目标应用时会继续在所有窗口里找，找不到才收起；
- **发送按钮只要「可见」**：QQ 在输入框为空时会把真正的发送键置为 disabled，而它相册面板里还有
  一个隐藏的同名 `send_btn` —— 所以「可见」这条是必须的，「可用」不能要；
- 发送键瞬时找不到（布局中、动画中、键盘弹起中）不会立刻收起，连续两次找不到才隐藏；
- 为了让按钮跟着发送按钮走，无障碍服务会监听事件 —— 但**只用于定位**，输入框文本仍然只在用户
  点按钮的那一刻才读取，这一点没有变。

配置在 **悬浮窗 → 发送按钮助手**，和悬浮窗本体放在一起 —— 它俩本来就是同一类东西：由本应用画在
别的应用之上的浮层，连**取色来源都是共用的**那一个。

**每个应用一条**，跟上面的悬浮窗按钮同一种形式：一行一个应用（微信、QQ），行尾一个开关直接
开关，点行打开它自己的设置。两个软件互不影响 —— 它们的输入栏形状不同，一个在微信上摆得好看的
尺寸，放到 QQ 上可能正好压住发送键。

| 项 | 范围 |
| --- | --- |
| 启用 | 开 / 关（每个应用各自一份） |
| 点它做什么 | AI 修改 / 复制文本 / 导出界面元素 |
| 做完自动发送 | 开 / 关（默认关） |
| 大小 | 24–72 dp |
| 圆角 | 0–短边/2 dp |
| 不透明度 | 30–100% |
...
**编辑器顶部是实时预览**（`ui/SendAssistPreview.kt`）：一个假的输入栏加一个假的「发送」键，
助手按钮就画在它上方，尺寸、圆角、透明度、偏移和真实浮层用的是同一套几何与同一份调色板。
调滑条时预览当场跟着变，不用切到微信看一眼再切回来。

偏移是相对默认落点的增量（默认「与发送键右对齐、在其**上方 8 dp**」），**`+x` 向右、`+y` 向下**，
所以负数就是把按钮往上、往左挪。改这些设置时按钮会**当场重绘**，不用重开聊天应用 ——
无障碍服务监听 `send_assist` 偏好，变更后重算大小与位置。

**关于那个 8 dp，以及为什么代码里写的是 56**：发送键的位置来自 `getBoundsInScreen`（从屏幕顶端算），
而浮层窗口是在**从状态栏下面开始**的坐标空间里摆放的 —— 于是「上方 8 dp」这个要求在真机上被系统吃掉
了大约一个状态栏（~48 dp），按钮不是悬在发送键上方，而是**压在发送键上**（微信、QQ 都一样，因为这是
系统行为）。这里不去探测、也不去纠正那个偏移，而是**把默认落点直接抬高**：

- 摆放时按 `RESTING_GAP_DP = 56f` 要位置（想要的 8 + 会被吃掉的 ~48），真机上就落在**上方 8 dp**；
- 预览画的是 `VISIBLE_CLEARANCE_DP = 8f`，也就是**用户真正看到的那 8 dp** —— 画「要的数」而不是画
  「落到的数」，正是之前预览和真机对不上的原因；
- 两个常量并排放在 `sendassist/SendAssist.kt` 里，因为一旦它们走散，预览就不再描述真机；
- 换一台「吃掉的量不同」的手机不用改代码：**垂直偏移滑条范围是 ±64 dp**，比任何状态栏都大，拖一下就
  能对齐；如果你告诉我该往哪边调多少，我可以直接改这两个常量。

偏好存在独立的 `send_assist` 文件里，因此也在配置备份的范围内。
