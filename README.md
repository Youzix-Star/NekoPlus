...
**编辑器顶部是实时预览**（`ui/SendAssistPreview.kt`）：助手按钮就画在输入栏的发送键上方。这里的
关键是**预览不自己算几何** —— 它和真机调用的是同一个
`SendAssistGeometry.assistantRect`（`sendassist/SendAssistGeometry.kt`），只是真机传像素、预览传
dp，所以「预览和实际有出入」在结构上就不该发生。

**预览画的是你手机上真实的那条输入栏**，不是通用示意：助手每次在聊天应用里定位时，都会顺手记下
发送键的真实尺寸、到屏幕右边缘的真实距离、输入框的真实高度、以及下方是不是键盘
（`SendAssistMetrics`，存在 `send_assist` 里）。所以标题会写清来源与时间：

- `预览 · 微信 实测：发送键 58×36 dp · 距右 12 dp · 输入框高 44 dp · 刚刚`
- 还没进过该应用时退化为 `预览 · 尚未进过微信，按通用输入栏示意`，并且**明说是示意**；
- 输入框只认「和发送键同一行」的可编辑节点 —— 微信聊天列表页顶部那个搜索框否则会被量成输入栏，
  预览就会在屏幕顶端画一条栏。

**「0 dp」到底长什么样，是量出来的**：发送按钮的位置来自 `getBoundsInScreen`（屏幕坐标），而这个
浮层窗口是放在窗口坐标里的 —— 两者起点不一致时（系统给窗口插了边，就是最常见的情形），按钮的**实际
落点会和算式差一个固定的量**。这里**不去纠正它**：那个落点就是用户已经习惯、并且正在用偏移去微调的
位置，替用户搬走它属于没人要求过的改动。改成**量它**：

- 按钮放好后，用 `getLocationOnScreen` 读回真实落点，算出「按钮下边缘到发送键上边缘」的真实间隙
  （用户自己的偏移会被减掉，量的是「设定 vs 实际」的差，不是用户的调参）；
- 差值超过 240 px 就判定为「窗口还没布局完」而丢弃，不写进记录；
- 结果存进该应用的实测记录，**预览用它来画**，所以预览里的 0 dp 和微信里的 0 dp 是同一件事；
- 标题里会直接说出来：「按钮实际比设定低 48 dp，已按实际画」—— 与其让两边悄悄不一致，不如把数字
  摆在脸上。不想让它低这 48 dp，就把垂直偏移调 −48。

这样即使换一台状态栏高度不同的手机，数字也会自己重新量，不需要任何硬编码的魔数。

窗口比手机窄时**从左边裁**而不是缩放（几何锚在屏幕右侧），这样每一个数字屏幕上仍是真实 dp，
尺寸才判断得准；只有偏移大到按钮离发送键很远、两者放不进同一张图时才整体缩小，并在标题里标出
百分比 —— 无声的缩放等于另一种撒谎。

偏移是相对默认落点的增量（默认「与发送键右对齐、在其上方 8 dp」），**`+x` 向右、`+y` 向下**，
所以负数就是把按钮往上、往左挪。改这些设置时按钮会**当场重绘**，不用重开聊天应用 ——
无障碍服务监听 `send_assist` 偏好，变更后重算大小与位置。

实测出来的输入栏尺寸与实际落点间隙都会打印在调试抓取里：

```
助手窗口: 实测 mm 58×36 dp/间隙 -40 dp、mobileqq 60×34 dp/间隙 未实测
```
