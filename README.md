...
**窗口坐标这件事踩过一次坑，值得记下来**：发送按钮的位置来自 `getBoundsInScreen`，是**全屏坐标**；
而没有声明 `FLAG_LAYOUT_IN_SCREEN` 的窗口是放在**内容区**里的，内容区从状态栏下面开始。于是算出来
的位置被系统整体往下推了一个状态栏的高度 —— 按钮没有悬在发送键上方 8 dp，而是直接压在发送键上
（两个应用都是，因为这是系统级的行为，不是某个应用的布局）。现在窗口声明了该 flag，并且**不相信
自己的算术**：窗口刚加上去时用 `getLocationOnScreen` 实测一次实际落点，差多少就补多少
（差得离谱时会拒绝相信这个答案，那是窗口还没布局完）。修正值会打印在调试抓取里：

```
助手窗口: 窗口修正 无 · 实测 mm 58×36 dp
```
