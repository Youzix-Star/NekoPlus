...
**页面**（被用户连骂两轮"界面混乱、多一个按钮是神经病"之后定的形状）：
...
- **点规则那一行就是编辑那条规则**，所以页面上不再有「表单编辑」按钮 —— 点得进去，按钮就是多余的；
- **新增规则**：一个粘贴框 + [粘贴] + [添加]。从别处复制一行（或多行）规则，粘贴、添加，
  它就直接进列表；加完还想细调，点它进表单慢慢改；
- **变量**在页面上，不在按钮后面：猫爪是"可自定义配置项"，藏起来等于没做；
- **文本编辑**：整份配置的文字，底部一个**试跑**框（写一句，下面是套用后的结果，跟着你正在输入
  的文字走，不是已保存的）。三个模板按钮**等宽等高**（标签长短不同会让按钮高度不同，这是被指出来的）；
- **规则包**：整包载入（内置猫化、猫爪）、保存当前、删除，也可以在 1.1.8 有旧设置时一键导入。

单条规则的表单仍然只有必要控件：条件下拉、条件值、动作下拉、参数、空格不加、只在哪些应用生效。**
