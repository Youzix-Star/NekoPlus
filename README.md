...
**页面只有三样东西**（第一版把开关、列表、两个入口、规则包、导入、试跑全堆在一页上，被用户骂了
「界面混乱」，于是按剃刀重写）：
...
**表单编辑**是一个对话框的**三个面**，不是三个对话框：

1. **规则列表**：点一条改一条、右边删除、下面「添加一条规则」，再往下是**变量**输入框
   （`{后缀}`、`{猫爪}`、`{颜文字}`… 改猫爪就是改那一格）；底部一行进「规则包」或「从 1.1.8 导入」；
2. **单条规则的表单**：条件下拉（总是/含/不含/以…开头/以…结尾/是/匹配正则/长度/随机/每 N 次）、
   条件里的字或数值、动作下拉（替换/删除/开头加/末尾加/每句末尾加/首尾包裹/去掉末尾标点/加颜文字/
   正则替换）、参数、空格不加、只在哪些应用生效（包名）。**完全不用碰语法**；
3. **规则包**：内置「猫化」「猫爪」一点即用；已存的包点按载入、右侧删除；下面一个名字框 +
   「保存当前规则」。

**文本编辑**是整份配置的文字（可粘贴、可分享、坏行按行号提示），底部一个**试跑**框：写一句，
下面立刻是套用后的结果 —— 用的是独立引擎，所以试跑不会推进「每 N 次」的计数。
