...
**两种编辑方式，同一份配置 —— 页面上就是两个按钮**（之前的表单编辑只能靠「点规则那一行」进去，
等于没有入口，这是被用户骂过之后改的）：

- **表单编辑**：打开的是一个对话框，先看到规则列表（加一条 / 点进去改 / 删除），下面直接编辑
  变量（`{后缀}`、`{猫爪}`、`{颜文字}`… 都是输入框）。点进去是单条规则的表单：条件下拉（总是/含/不含/以…开头/以…结尾/是/匹配正则/长度/随机/每 N 次）、
  反转开关、动作下拉（替换/删除/开头加/末尾加/每句末尾加/首尾包裹/去掉末尾标点/加颜文字/正则替换）、
  参数输入框、空格不加、只替换第一个、生效范围。**完全不用碰语法**；
- **文本编辑**：整份配置的文字，可粘贴、可分享、坏行按行号报错，旁边一排模板。
  两种方式改的是同一份模型，所以随时可以来回换。规则列表里点某一行，也是直接进那一条的表单。
