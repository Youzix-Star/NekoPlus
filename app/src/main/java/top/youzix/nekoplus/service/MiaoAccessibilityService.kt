...
open class MiaoAccessibilityService : AccessibilityService() {
    ...
    private fun syncSendAssist() {
        if (top.youzix.nekoplus.sendassist.SendAssistPrefs.anyEnabled(this)) {
    ...
    override fun onServiceConnected() {
        ...
        top.youzix.nekoplus.sendassist.SendAssistPrefs.register(this, sendAssistPrefs)
        syncSendAssist()
    }
    ...
    override fun onDestroy() {
        sendAssist?.dispose()
        sendAssist = null
        runCatching { top.youzix.nekoplus.sendassist.SendAssistPrefs.unregister(this, sendAssistPrefs) }
        instance = null
        super.onDestroy()
    }
    ...
    fun dumpScreen(): String {
        ...
        return buildString {
            ...
            appendLine(
                "发送按钮助手: " + top.youzix.nekoplus.sendassist.SEND_TARGETS.joinToString("、") { target ->
                    val config = top.youzix.nekoplus.sendassist.SendAssistPrefs.load(
            ...
            active?.packageName?.toString()?.let { pkg ->
                val target = top.youzix.nekoplus.sendassist.sendTargetFor(pkg)
                if (target != null) {
                        ...
                        "  已适配的应用，按 id 找到的发送按钮: " +
                            (
                                button?.let { describe(it) } ?: "无（会退化为按文字「" +
                                    top.youzix.nekoplus.sendassist.SEND_LABEL + "」查找）"
                                ),
                    )
                }
            }
